...
//{
//	img.clear();
//	img.push_back(cvLoadImage(simage_repath[3].c_str())); //这里对创建的金字塔层中第一层(金字塔是从第0层开始创建的)进行的配准；；之所以不采用对第一层进行初配准，原因是这两种操作达到的目的都是获取重叠区域感兴趣的部分，但是时间却不一样，而且，在遇到很大的图像的时候，即使是第0层图像也无法用opencv来处理。综合来讲，使用第1层是最安全且有效的。
//	img.push_back(cvLoadImage(simage_senpath[3].c_str())); 
...
//			
//			cvSaveImage( resultImage_path.c_str(), xformed );
//
//			//在此处调用显示结果图像的函数
//			//OpenResultFile();
//			//Cut_Count_Overlap();
...
//		}
//
//		///no use///以上部分为原来SURF算法中寻找H矩阵的部分，
//		///no use///下面要做的是对匹配的特征点进行挑选，然后进行大图的匹配,
//		///no use////其中，挑选之前的特征点保存在ptpairs_before,挑选之后的特征点保存在ptpairs_after
//		///no use////同时注意，ptpairs中存储的仅仅是特征点序号的序列。所以可能还需对存储特征点的描述子进行刷新
//	}
//}
...
void Cut_Count_Overlap()
{
	...
	////画出重叠区域的矩形框
	//cvRectangle(image0,cvPoint(h1,h2),cvPoint(image1_xsize,image1_ysize),cvScalar(0,0,255),3,4,0 );
	//cvNamedWindow("OverLapArea",CV_WINDOW_AUTOSIZE);
	//cvShowImage("Image0WithOverLapArea",image0);
	//cvWaitKey(0);
...
//				rectangle(image0,cvPoint(h1,h2),cvPoint(h1+hx,h2+hy),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);

				////cvNamedWindow("OverLapArea0",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window0,image0);
...
//				rectangle(image0,cvPoint(h1,h2+hy),cvPoint(h1+hx,image1_ysize-hy),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);

				////cvNamedWindow("OverLapArea3",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window3,image0);

			}
			else if (r1->pt.y > image1_ysize-hy && r1->pt.y <= image1_ysize  )
			{
				areaCount1[6]++;
//				rectangle(image0,cvPoint(h1,image1_ysize-hy),cvPoint(h1+hx,image1_ysize),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);

				////cvNamedWindow("OverLapArea6",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window6,image0);
			}
		...
		else if ( r1->pt.x > h1+hx && r1->pt.x <= image1_xsize-hx )
		{
			...
			{
				areaCount1[1]++;
//				rectangle(image0,cvPoint(h1+hx,h2),cvPoint(image1_xsize-hx,image1_ysize),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);
				////cvNamedWindow("OverLapArea1",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window1,image0);
			} 
			else if(r1->pt.y > (h2+hy) && r1->pt.y <= (image1_ysize-hy) )
			{
				areaCount1[4]++;
				//rectangle(image0,cvPoint(h1+hx,h2+hy),cvPoint(image1_xsize-hx,image1_ysize-hy),cvScalar(255,0,255),2,4,0 );
				//circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);
				////cvNamedWindow("OverLapArea4",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window4,image0);
			}
			else if (r1->pt.y > image1_ysize-hy && r1->pt.y <= image1_ysize  )
			{
				areaCount1[7]++;
//				rectangle(image0,cvPoint(h1+hx,image1_ysize-hy),cvPoint(image1_xsize-hx,image1_ysize),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);
				////cvNamedWindow("OverLapArea7",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window7,image0);
			}
		...
		else if ( r1->pt.x > image1_xsize-hx && r1->pt.x <= image1_xsize )
		{
			...
			{
				areaCount1[2]++;
//				rectangle(image0,cvPoint(image1_xsize-hx,h2),cvPoint(image1_xsize,image1_ysize),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);
				////cvNamedWindow("OverLapArea2",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window2,image0);
			} 
			else if(r1->pt.y > (h2+hy) && r1->pt.y <= (image1_ysize-hy) )
			{
				areaCount1[5]++;
//				rectangle(image0,cvPoint(image1_xsize-hx,h2+hy),cvPoint(image1_xsize,image1_ysize-hy),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);
				////cvNamedWindow("OverLapArea5",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window5,image0);
			}
			else if (r1->pt.y > image1_ysize-hy && r1->pt.y <= image1_ysize  )
			{
				areaCount1[8]++;
//				rectangle(image0,cvPoint(image1_xsize-hx,image1_ysize-hy),cvPoint(image1_xsize,image1_ysize),cvScalar(255,0,255),2,4,0 );
//				circle(image0,r1->pt,3,cvScalar(0,255,255),-1,8);
				////cvNamedWindow("OverLapArea8",CV_WINDOW_AUTOSIZE);
				//imshow(atom_window8,image0);
			}
		}
...
// 	itoa(maxnum,ch,10);
	...
	//rectangle(image0,cvPoint(h1,h2),cvPoint(h1+hx,h2+hy),cvScalar(0,0,255),3,4,0 );
	
	switch (maxnum1){
	case 0:
		//参考图像的裁剪
		CvPoint2D64f point0;
		point0.x= 16*h1;///不行，应该把这些数都改为浮点数
		point0.y = 16*h2;
		rePoint.push_back(point0);

	//	rectangle(image0,cvPoint(h1,h2),cvPoint(h1+hx,h2+hy),cvScalar(0,255,255),2,4,0 );
	//	imshow(atom_window0,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point0.x,point0.y,16*hx,16*hy,"GTiff");
	...
	//	rectangle(image1,cvPoint(0,0),cvPoint(hx,hy),cvScalar(255,255,0),2,4,0 );
	//	imshow(atom1_window0,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),0,0,16*hx,16*hy,"GTiff");

		//waitKey(0);
		break;
	case 1:
		CvPoint2D64f point1;
		point1.x = 16*(h1+hx);///不行，应该把这些数都改为浮点数
		point1.y = 16*h2;
		rePoint.push_back(point1);

	//	rectangle(image0,cvPoint(h1+hx,h2),cvPoint(image1_xsize-hx,h2+hy),cvScalar(0,255,255),2,4,0 );
	//	imshow(atom_window1,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point1.x,point1.y,16*(image1_xsize-2*hx),16*hy,"GTiff");


		CvPoint2D64f point11;
		point11.x = 16*hx;///不行，应该把这些数都改为浮点数
		point11.y = 0;
		senPoint.push_back(point11);

	//	rectangle(image1,cvPoint(hx,0),cvPoint(dx-hx,hy),cvScalar(255,255,0),2,4,0 );
	//	imshow(atom1_window1,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point11.x,point11.y,16*(dx-hx*2),16*hy,"GTiff");
	...
	case 2:
		CvPoint2D64f point2;
		point2.x =16*(image1_xsize-hx);///不行，应该把这些数都改为浮点数
		point2.y = 16*h2;
		rePoint.push_back(point2);

	//	rectangle(image0,cvPoint(image1_xsize-hx,h2),cvPoint(image1_xsize,h2+hy),cvScalar(0,255,255),2,4,0 );
	//	imshow(atom_window2,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point2.x,point2.y,16*hx,16*hy,"GTiff");


		CvPoint2D64f point22;
		point22.x = 16*(dx-hx);///不行，应该把这些数都改为浮点数
		point22.y = 0;
		senPoint.push_back(point22);

	//	rectangle(image1,cvPoint(dx-hx,0),cvPoint(dx,hy),cvScalar(255,255,0),2,4,0 );
	//	imshow(atom1_window2,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point22.x,point22.y,16*hx,16*hy,"GTiff");
	...
	case 3:
		CvPoint2D64f point3;
		point3.x = 16*h1;///不行，应该把这些数都改为浮点数
		point3.y = 16*(h2+hy);
		rePoint.push_back(point3);

	//	rectangle(image0,cvPoint(h1,h2+hy),cvPoint(h1+hx,image1_ysize-hy),cvScalar(0,255,255),2,4,0 );
	//	imshow(atom_window3,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point3.x,point3.y,16*hx,16*hy,"GTiff");
	...
	//	rectangle(image1,cvPoint(0,hy),cvPoint(hx,dy-hy),cvScalar(255,255,0),2,4,0 );
	//	imshow(atom1_window3,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point33.x,point33.y,16*hx,16*(dy-2*hy),"GTiff");
	...
	case 4:	
		CvPoint2D64f point4;
		point4.x = 16*(h1+hx);///不行，应该把这些数都改为浮点数
		point4.y = 16*(h2+hy);
		rePoint.push_back(point4);

	//	rectangle(image0,cvPoint(h1+hx,h2+hy),cvPoint(image1_xsize-hx,image1_ysize-hy),cvScalar(0,255,255),2,4,0 );
	//	imshow(atom_window4,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point4.x,point4.y,16*(image1_xsize-2*hx),16*(image1_ysize-2*hy),"GTiff");

		CvPoint2D64f point44;
		point44.x = 16*hx;///不行，应该把这些数都改为浮点数
		point44.y = 16*hy;
		senPoint.push_back(point44);

//		rectangle(image1,cvPoint(hx,hy),cvPoint(dx-hx,dy-hy),cvScalar(255,255,0),2,4,0 );
//		imshow(atom1_window4,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point44.x,point44.y,16*(dx-2*hx),16*(dy-2*hy),"GTiff");
	...
	case 5:
		CvPoint2D64f point5;
		point5.x = 16*(image1_xsize-hx);///不行，应该把这些数都改为浮点数
		point5.y = 16*(h2+hy);
		rePoint.push_back(point5);

//		rectangle(image0,cvPoint(image1_xsize-hx,h2+hy),cvPoint(image1_xsize,image1_ysize-hy),cvScalar(0,255,255),2,4,0 );
//		imshow(atom_window5,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point5.x,point5.y,16*hx,16*(image1_ysize-2*hy),"GTiff");

		CvPoint2D64f point55;
		point55.x = 16*(dx-hx);///不行，应该把这些数都改为浮点数
		point55.y = 16*hy;
		senPoint.push_back(point55);
//		rectangle(image1,cvPoint(dx-hx,hy),cvPoint(dx,dy-hy),cvScalar(255,255,0),2,4,0 );
//		imshow(atom1_window5,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point55.x,point55.y,16*hx,16*(dy-2*hy),"GTiff");
	...
	case 6:
		CvPoint2D64f point6;
		point6.x = 16*h1;///不行，应该把这些数都改为浮点数
		point6.y = 16*(image1_ysize-hy);
		rePoint.push_back(point6);

//		rectangle(image0,cvPoint(h1,image1_ysize-hy),cvPoint(h1+hx,image1_ysize),cvScalar(0,255,255),2,4,0 );
//		imshow(atom_window6,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point6.x,point6.y,16*hx,16*hy,"GTiff");

		CvPoint2D64f point66;
		point66.x = 0;///不行，应该把这些数都改为浮点数
		point66.y = 16*(dy-hy);
		senPoint.push_back(point66);
//		rectangle(image1,cvPoint(0,dy-hy),cvPoint(hx,dy),cvScalar(255,255,0),2,4,0 );
//		imshow(atom1_window6,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point66.x,point66.y,16*hx,16*hy,"GTiff");

	//	waitKey(0);
		break;
	case 7:
		CvPoint2D64f point7;
		point7.x = 16*(h1+hx);///不行，应该把这些数都改为浮点数
		point7.y = 16*(image1_ysize-hy);
		rePoint.push_back(point7);

//		rectangle(image0,cvPoint(h1+hx,image1_ysize-hy),cvPoint(image1_xsize-hx,image1_ysize),cvScalar(0,255,255),2,4,0 );
//		imshow(atom_window7,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point7.x,point7.y,16*(image1_xsize-2*hx),16*hy,"GTiff");

		CvPoint2D64f point77;
		point77.x = 16*hx;///不行，应该把这些数都改为浮点数
		point77.y = 16*(dy-hy);
		senPoint.push_back(point77);
//		rectangle(image1,cvPoint(hx,dy-hy),cvPoint(dx-hx,dy),cvScalar(255,255,0),2,4,0 );
//		imshow(atom1_window7,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point77.x,point77.y,16*(dx-2*hx),16*hy,"GTiff");
	...
	case 8:
		CvPoint2D64f point8;
		point8.x = 16*(image1_xsize-hx);///不行，应该把这些数都改为浮点数
		point8.y = 16*(image1_ysize-hy);
		rePoint.push_back(point8);

	//	rectangle(image0,cvPoint(image1_xsize-hx,image1_ysize-hy),cvPoint(image1_xsize,image1_ysize),cvScalar(0,255,255),2,4,0 );
	//	imshow(atom_window8,image0);
		ImageCut(simage_repath[0].c_str(),Overlap1.c_str(),point8.x,point8.y,16*hx,16*hy,"GTiff");

		CvPoint2D64f point88;
		point88.x = 16*(dx-hx);///不行，应该把这些数都改为浮点数
		point88.y = 16*(dy-hy);
		senPoint.push_back(point88);
	//	rectangle(image1,cvPoint(dx-hx,dy-hy),cvPoint(dx,dy),cvScalar(255,255,0),2,4,0 );
	//	imshow(atom1_window8,image1);
		ImageCut(simage_senpath[0].c_str(),Overlap2.c_str(),point88.x,point88.y,16*hx,16*hy,"GTiff");
...
void sy_1::BigMapRegistration()
{
	...
	string sImageResultPath = dir + "PyramidImageRegResult.jpg";
	string BigMapResult = dir + "BigMapRegistrationResult.tif";
	
	ResultImagePath.clear(); 
	
	ResultImagePath.push_back(dir);
	...
	SURF_ON_TWO(simage_repath[4],simage_senpath[4],sImageResultPath);//调取金字塔第1层图像进行匹配
